...
if($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    if(!count($errors)){
        ...
        if(!$userData) {
            query("INSERT INTO users (firstName,lastName,email,password,userImage) VALUES(:firstName,:lastName,:email,:password,:image)",[
                "firstName" => $firstName,
                "lastName" => $lastName,
                "email" => $email,
                "password" => $password,
                "image" => publicImage("person.png")
            ]);
            $userData = getUserByEmail($email);
            createStudyProgress($userData['userID']);
            createExampleProgress($userData['userID']);
            redirect("/login");
        } else {
            $errors = [
